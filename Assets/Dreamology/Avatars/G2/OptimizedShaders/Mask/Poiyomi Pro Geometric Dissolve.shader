Shader "Hidden/Locked/.poiyomi/Poiyomi 8.2/Poiyomi Pro Geometric Dissolve/e01031a1c19e6ea478e90037682989f1"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void GeometryProgram(triangle VertexOut p[3], inout TriangleStream<VertexOut> triStream)
			{
				...
				float geoDissolveAlpha = (0.072 /*_GeoDissolveAlpha*/);
				...
				else if ((2.0 /*_GeoDissolveGradientType*/) == GEO_DISSOLVE_NOISE)
				{
					...
					float a = 0.0;
					float2 vel = _Time.x * (15.0 /*_GeoDissolveNoiseSpeed*/);
					...
					a = snoise(uvToUse * float2(cos(_Time.x * (15.0 /*_GeoDissolveNoiseSpeed*/) * 0.15), sin(_Time.x * (15.0 /*_GeoDissolveNoiseSpeed*/) * 0.1)) * 0.1) * 3.1415;
					vel = float2(cos(a), sin(a));
					geoDissolveAlpha += snoise(uvToUse + vel) * .25 + .25;
					geoDissolveAlpha = 1 - (geoDissolveAlpha);
					geoDissolveAlpha = remap(geoDissolveAlpha, (0.0 /*_GeoDissolveNoiseThreshold*/) - (1 - (0.0 /*_GeoDissolveNoiseThreshold*/)), 1, -1 + (saturate(baseGeoDissolveAlpha) * 2), 1 - (saturate(-1 * (0.072 /*_GeoDissolveAlpha*/)) * 2));
				}
				...
				if (geoDissolveAlpha < 1.0)
				{
					...
					for (int i = 0; i < 3; i++)
					{
						...
						p[i].localPos.xyz += outDir * geoDissolveAlpha * (0.01 /*_GeoDissolveTravelDistance*/);
					...
					if ((1.0 /*_GeoDissolveShapeType*/) == GEO_DISSOLVE_SHAPE_PYRAMID && geoDissolveAlpha > 0)
					{
						...
						o4.localPos.xyz += outDir * geoDissolveAlpha * (0.01 /*_GeoDissolveTravelDistance*/);
			...
			{
				float4 geoDissolveColor = float4(poiThemeColor(poiMods, float4(1,1,1,0.254902).rgb, (0.0 /*_GeoDissolveColorThemeIndex*/)), float4(1,1,1,0.254902).a);
				#if defined(PROP_GEODISSOLVETEXTURE) || !defined(OPTIMIZER_ENABLED)
			...
			float3 applyEmission3(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionStrength3 = (2.32 /*_EmissionStrength3*/);
				...
				{
					emissionColor3 = POI2D_SAMPLER_PAN(_EmissionMap3, _MainTex, poiUV(poiMesh.uv[(0.0 /*_EmissionMap3UV*/)], float4(1,1,0,0)), float4(0,0,0,0)) * lerp(1, poiFragData.baseColor, (0.0 /*_EmissionBaseColorAsMap3*/)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, (10.0 /*_EmissionColor3ThemeIndex*/));
				}
				else
				{
					emissionColor3 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap3, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * (5.0 /*_EmissionCenterOutSpeed3*/)).rgb * lerp(1, poiFragData.baseColor, (0.0 /*_EmissionBaseColorAsMap3*/)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, (10.0 /*_EmissionColor3ThemeIndex*/));
				}
				#else
				emissionColor3 = lerp(1, poiFragData.baseColor, (0.0 /*_EmissionBaseColorAsMap3*/)).rgb * poiThemeColor(poiMods, float4(1,1,1,1).rgb, (10.0 /*_EmissionColor3ThemeIndex*/));
				#endif
				...
				applyLumaGradient(poiMods, emissionColor3, (10.0 /*_EmissionColor3ThemeIndex*/), poiLight.nDotV);
